"""
Train a Mamba-1 (or S-SSSM) CTC acoustic encoder on LibriSpeech.

Part of the thesis: "State Space Models for Automatic Speech Transcription"
    Robert Alexander Hanssen, University of Agder, 2026

References:
    Gu & Dao, "Mamba: Linear-Time Sequence Modeling with Selective State Spaces",
    arXiv:2312.00752, 2023.

Supports two encoder backends selected via --encoder-type:
    mamba  – True Mamba with ZOH discretization and optional CUDA kernels
    sssm   – Original S-SSSM with input-dependent a/b/c/d and optional gating

Both encoders support non-uniform layer-wise initialization of decay/timescale
parameters (enabled by default; disable with --no-hierarchical).  The flag name
"--no-hierarchical" is kept for compatibility with thesis experiment commands;
in the final thesis this is described as "non-uniform layer-wise initialization"
or "initialization diversity."

Setup:
    pip install lightning torch torchaudio jiwer matplotlib numpy pandas datasets
    # For CUDA-accelerated Mamba (10-100x faster, optional):
    pip install mamba-ssm>=2.0.0 causal-conv1d>=1.4.0   # Linux, NVIDIA GPU, CUDA 11.6+

Expected data layout (HuggingFace datasets saved with save_to_disk):
    hub_data/librispeech/clean/   -> train.100, train.360, validation, test
    hub_data/librispeech/other/   -> train.500, validation, test

Usage examples:
    # Quick smoke test
    python IKT590_train_ASR_encoder_mamba-1.py \\
        --subset-train 2000 --subset-val 200 --epochs 3 --d-model 128 --n-layers 6

    # 100h Mamba with CUDA kernels (default)
    python IKT590_train_ASR_encoder_mamba-1.py \\
        --data-path hub_data/librispeech --dataset-config 100h \\
        --d-model 256 --n-layers 12 --d-state 16 --seed 456 --epochs 30

    # Uniform init ablation (disable non-uniform init)
    python IKT590_train_ASR_encoder_mamba-1.py \\
        --data-path hub_data/librispeech --no-hierarchical --no-gating

    # Force pure-PyTorch fallback (no CUDA kernels)
    python IKT590_train_ASR_encoder_mamba-1.py --no-cuda-kernels
...
print("\n")
print("SSM-ASR: Mamba-1 CTC Encoder Training")
print("\n")
...
# ============================================================================
# NON-UNIFORM INITIALIZATION CONFIGURATION (Module Level)
# ============================================================================
# Legacy name "HIER_CONFIG" kept for checkpoint/config compatibility.
# In the final thesis, this is referred to as "non-uniform layer-wise
# initialization" or "initialization diversity."
...
HIER_CONFIG = {
    ...
    'B0_LATE':  0.12,   # Lower sensitivity in later layers
...
MAMBA_HIER_CONFIG = {
    # dt (timescale) initialization range — non-uniform schedule.
    # Early layers: smaller dt → faster response to input changes
    # Late layers: larger dt → slower, more integrative response
    'DT_MIN_EARLY': 0.001,    # 1ms
    'DT_MAX_EARLY': 0.01,     # 10ms
    'DT_MIN_LATE': 0.005,     # 5ms
    'DT_MAX_LATE': 0.05,      # 50ms
    # A_log scaling factors for pure-PyTorch Mamba fallback path
    'A_SCALE_EARLY': 0.8,
    'A_SCALE_LATE': 1.2,
}
...
class MambaLayer(nn.Module):
    """
    Mamba layer with optional non-uniform layer-wise initialization.
    ...
    Non-uniform initialization (when use_hierarchical=True) modifies:
        1. dt_bias: Layer-depth-dependent timescale initialization
        2. A_log: Optional scaling in the PyTorch fallback path
    
    Note: The parameter name "use_hierarchical" is kept for compatibility
    with saved configs and checkpoint commands.  In the final thesis this
    corresponds to non-uniform layer-wise initialization / init diversity.
    """
    ...
    def __init__(
        ...
        # Positional label for logging only (early/mid/late); not a claim
        # about learned function.
    ...
    def _apply_hierarchical_init_cuda(self):
        """
        Apply non-uniform layer-wise initialization that ONLY modifies dt_bias.
        
        Preserves Mamba's full frequency spectrum in A_log while biasing
        which timescales each layer prefers at initialization.
        
        Forward schedule: early layers get smaller dt (faster response),
        later layers get larger dt (more integrative).
        """
        ...
        with torch.no_grad():
            ...
            # ============================================
            # ONLY modify dt_bias for non-uniform init effect
            # ============================================
...
class MambaEncoder(nn.Module):
    """Stack of Mamba layers with optional non-uniform init and CUDA support."""
    ...
    def __init__(
        ...
        if use_hierarchical:
            print(f"\nNon-uniform Init (dt_bias layer-wise schedule):")
            print(f"  {'Layer':<6} {'Position':<11} {'A_scale':<10} {'dt_mean':<12}")
            print(f"  {'-'*6} {'-'*11} {'-'*10} {'-'*12}")
...
class ConvSubsample(nn.Module):
    """4x subsampling via two strided convolutions."""
...
class HierarchicalSelectiveSSMLayer(nn.Module):
    """
    Selective SSM Layer with optional non-uniform init and gating.
    
    Class name kept for checkpoint compatibility.  "Hierarchical" here
    corresponds to non-uniform layer-wise initialization in the thesis.
    """
    ...
    def __init__(
        ...
        if use_hierarchical:
            with torch.no_grad():
                ...
                # Positional label for logging (not a claim about learned role)
...
class SSMEncoder(nn.Module):
    ...
    def __init__(
        ...
        if use_hierarchical:
            ...
            print(f"\nNon-uniform Init (Target Means, forward schedule):")
            print(f"   Early layers:  a0≈{a_early:.2f} (lower initial decay)")
            print(f"   Mid layers:    a0≈{a_mid:.2f}")
            print(f"   Late layers:   a0≈{a_late:.2f} (higher initial decay)\n")
            
            print(f"  {'Layer':<6} {'Position':<11} {'a0 (mean±std)':<20} {'b0 (mean±std)':<20} {'c0 (mean±std)':<20}")
...
class ASRModel(nn.Module):
    ...
    Supports:
    - 'mamba': True Mamba architecture with non-uniform layer-wise init
...
class HierarchyFreezeCallback(Callback):
    ...
    to force the model to utilise the imposed layer-wise timescale diversity,
    then unfreezes them to allow fine-tuning.
    
    Class name kept for compatibility.
    """
...
def train(config: Config):
    ...
    # Conditionally add the freeze callback
    ...
    # 3. Plot learned layer-wise dynamics
    ...
    if hasattr(model, 'final_detailed_results') and model.final_detailed_results:
        ...
        visualizer.plot_example_predictions(examples[:5])
    else:
...
class Visualizer:
    ...
    def plot_ablation_study(
        ...
        """
        if configurations is None or wer_values is None:
            print("  Skipped ablation_study.pdf: pass configurations and wer_values explicitly")
            return
    ...
    def plot_hierarchical_initialization(self, n_layers: int = 12):
            """Plot non-uniform initialization values across layers."""
            ...
            # These formulas match the logic in HierarchicalSelectiveSSMLayer.__init__
            ...
            axes[0].set_title('Non-uniform Init Schedule (Forward)')
    ...
    def visualize_hierarchical_dynamics(self, model):
        ...
        Method name kept for compatibility; generates hierarchical_dynamics.pdf."""
        ...
        axes[0].set_title(f'Learned Layer-wise Dynamics ({encoder_type.upper()})')
        axes[0].grid(True, alpha=0.3)
...
class Config:
    ...
    encoder_type: str = "mamba"  # "mamba" or "sssm"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train a Mamba-1 (or S-SSSM) CTC encoder on LibriSpeech"
    )
    ...
    parser.add_argument(
        '--no-hierarchical',
        action='store_true',
        help='Disable non-uniform layer-wise init (flag name kept for compat)'
    )
    ...
    parser.add_argument(
        ...
        default='mamba',
        choices=['mamba', 'sssm'],
        help='Encoder architecture: mamba (true Mamba) or sssm (original S-SSSM)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help='SSM state dimension for Mamba (typically 16 or 64)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help='Convolution kernel size for Mamba (typically 4)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help='Expansion factor for Mamba inner dimension (typically 2)'
    )
